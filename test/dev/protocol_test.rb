...
module Dev
  class ProtocolTest < Minitest::Spec
    ...
    ITEM_TEMPLATE = <<~ERB
      <ul><% @items.each do |item| %><%# herb:key item %><li data-pending="<%= pending %>" data-failed="<%= failed %>"><span><%= item.author %></span><span><%= item.body %></span></li><% end %></ul>
    ERB

    def compiled_entries(source)
    ...
    test "removing one span inside an item nulls only that span's slot" do
      ...
      old_entries = compiled_entries(before_source)
      new_entries = compiled_entries(after_source)
      operations = Herb.diff(before_source, after_source, track_whitespace_changes: true).operations

      remap = Herb::Dev::Protocol.remap(old_entries, new_entries, operations)
      nulled = remap["slots"].select { |_, target| target.nil? }.keys

      removed = old_entries.find { |entry| nulled.include?(entry[:index].to_s) }

      assert_equal 1, nulled.length
      assert_equal "child", removed[:type].to_s
    end

    test "an expression edit inside an item remaps every slot to itself" do
      before_source = ITEM_TEMPLATE
      after_source = ITEM_TEMPLATE.sub("item.body", "item.subject")
      ...
      identity = old_entries.to_h { |entry| [entry[:index].to_s, entry[:index]] }

      assert_equal identity, remap["slots"]
    end

    test "attribute slots sharing an element remap by their attribute" do
      entries = [
        { index: 0, type: "attribute", attribute: "data-pending", node_path: [0, 1], name: nil },
        { index: 1, type: "attribute", attribute: "data-failed", node_path: [0, 1], name: nil },
        { index: 2, type: "attribute", attribute: "data-starred", node_path: [0, 1], name: nil }
      ]

      remap = Herb::Dev::Protocol.remap(entries, entries, [])

      assert_equal({ "0" => 0, "1" => 1, "2" => 2 }, remap["slots"])
    end
