  ...
  module ActionView
    class LinkToTest < Minitest::Spec
      ...
      test "link_to with text and url" do
        assert_optimized_snapshot('<%= link_to "Click me", "#" %>')
      end

      test "link_to with html options" do
        assert_optimized_snapshot('<%= link_to "Click me", "#", class: "example" %>')
      end

      test "link_to with block" do
        assert_optimized_snapshot('<%= link_to "#" do %>Click me<% end %>')
      end

      test "link_to with block and class" do
        assert_optimized_snapshot('<%= link_to "#", class: "btn" do %>Click me<% end %>')
      end

      test "link_to with :back" do
        assert_optimized_snapshot('<%= link_to "Back", :back %>')
      end
      ...
      test "link_to with inline block and attributes" do
        assert_optimized_snapshot('<%= link_to("/about", class: "btn") { "About" } %>')
      end
      ...
      test "link_to with block containing HTML and object method call" do
        assert_optimized_snapshot(<<~ERB, { "@profile": Profile.new(id: 100, name: "Alice") })
          <%= link_to @profile do %>
