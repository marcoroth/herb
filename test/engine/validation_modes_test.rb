...
module Engine
  class ValidationModesTest < Minitest::Spec
    ...
    test ":overlay mode includes validation error details in HTML" do
      ...
      template_match = engine.src.match(/<template[^>]*data-herb-validation-error[^>]*>.*?<\/template>/m)
    ...
    test ":overlay mode with multiple validation errors" do
      ...
      templates = engine.src.scan(/<template[^>]*data-herb-validation-error[^>]*>.*?<\/template>/m)
