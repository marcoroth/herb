...
module Engine
  module Slots
    class BindingsTest < Minitest::Spec
      ...
      test "a splat is refused" do
        with_project { assert_match(/`\*\*extra` hides which states it binds/, refusal(caller_with("{ open: modal_open, **extra }"))) }
      end

      test "a splat on its own is refused" do
        with_project { assert_match(/`state:` spells every binding out as `name: value`/, refusal(caller_with("{ **extra }"))) }
      end

      test "a string key is refused" do
        with_project { assert_match(/`"open"` is not a state name/, refusal(caller_with("{ \"open\" => modal_open }"))) }
      end

      test "a refused splat points at the entry and not the whole render" do
        with_project do
          source = caller_with("{ open: modal_open, **extra }")
          location = assert_raises(Herb::Engine::CompilationError) { compile(source) }.diagnostics.fetch(0).location

          assert_equal "**extra", source.lines.fetch(location.start.line - 1)[location.start.column...location.end.column]
        end
      end
