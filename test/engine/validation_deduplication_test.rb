...
module Engine
  class ValidationDeduplicationTest < Minitest::Spec
    ...
    test "validation errors in ERB loops generate single template per location" do
      ...
      assert_compiled_snapshot(input, visitors: Herb::Engine::Validators.all(fatal: false), filename: "loop_test.html.erb")
    end
    ...
    test "multiple identical errors at different locations generate separate templates" do
      ...
      assert_compiled_snapshot(input, visitors: Herb::Engine::Validators.all(fatal: false), filename: "multi_test.html.erb")
    end

    test "validation overlay includes deduplication metadata" do
      input = "<div <%= @test %>></div>"

      assert_compiled_snapshot(input, visitors: Herb::Engine::Validators.all(fatal: false), filename: "meta_test.html.erb")
    end
  end
end
