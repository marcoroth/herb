...
module Engine
  class StaticTemplateOptimizationTest < Minitest::Spec
    ...
    def optimize(source, **)
      Herb::Engine.new(source, visitors: [Herb::Engine::Visitors::Optimize.new], **).src
    end
    ...
    test "an HTML-only template compiles to a bare string literal" do
      assert_compiled_snapshot("<div class=\"a\">Hello</div>", visitors: [Herb::Engine::Visitors::Optimize.new])
    end
    ...
    test "a recorded diagnostic keeps the buffer so the report survives" do
      compiled = Herb::Engine.new(
        "<div>hi</div>",
        visitors: [Herb::Engine::Visitors::Optimize.new, AlwaysWarns.new]
      ).src
    ...
    test "a fatal diagnostic still raises instead of collapsing" do
      assert_raises(Herb::Engine::CompilationError) do
        Herb::Engine.new(
          "<div>hi</div>",
          visitors: [Herb::Engine::Visitors::Optimize.new, AlwaysErrors.new]
        )
      end
    end
  end
