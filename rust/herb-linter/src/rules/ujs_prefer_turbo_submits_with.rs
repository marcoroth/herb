...
use crate::rules::ujs_base::{UJSAttributeDescriptor, UJSAttributeVisitor, UJSReplacement};
...
static DESCRIPTOR: LazyLock<UJSAttributeDescriptor> = LazyLock::new(|| UJSAttributeDescriptor {
  ...
  data_key: "disable_with",
  replacement: Some(UJSReplacement {
    attribute: "data-turbo-submits-with",
    option: "data: { turbo_submits_with: ... }",
  }),
  keyword: None,
});

pub struct UJSPreferTurboSubmitsWithRule;

impl Rule for UJSPreferTurboSubmitsWithRule {
  fn name(&self) -> &'static str {
    "ujs-prefer-turbo-submits-with"
  }
...
impl ParserRule for UJSPreferTurboSubmitsWithRule {
