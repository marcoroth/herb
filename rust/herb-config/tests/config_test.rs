...
mod config_exists {
  ...
  fn returns_true_when_config_file_exists() {
    ...
    fs::write(dir.path().join(".herb.yml"), "version: 0.10.4\n").unwrap();
  ...
  fn handles_explicit_herb_yml_path() {
    ...
    fs::write(&config_path, "version: 0.10.4\n").unwrap();
...
mod config_upgrade_workflow {
  ...
  fn version_can_be_updated_via_file_content_replacement() {
    ...
    let contents = fs::read_to_string(&config_path).unwrap().replace("version: 0.8.0", "version: 0.10.4");
    fs::write(&config_path, contents).unwrap();

    assert_eq!(Config::load(dir.path(), None).unwrap().config_version, Some("0.10.4".to_string()));
  }
}
