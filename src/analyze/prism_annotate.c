...
static token_T* get_content_token(const AST_NODE_T* node) {
  switch (node->type) {
    ...
    case AST_ERB_EACH_BLOCK_NODE: return ((AST_ERB_EACH_BLOCK_NODE_T*) node)->content;
...
static void set_prism_node(AST_NODE_T* node, herb_prism_node_T prism_ref) {
  switch (node->type) {
    ...
    case AST_ERB_EACH_BLOCK_NODE: ((AST_ERB_EACH_BLOCK_NODE_T*) node)->prism_node = prism_ref; break;
