...
static void append_unusable_state_entry(
  hb_array_T* entries,
  pm_node_t* element,
  const char* source,
  size_t erb_content_offset,
  ...
  pm_node_t* named = element->type == PM_ASSOC_NODE ? ((pm_assoc_node_t*) element)->key : element;
  char* name = extract_source_expression(named, allocator);
  ...
  position_T start = { .line = 1, .column = 1 };
  position_T end = start;

  if (source && erb_content_source) {
    size_t start_offset = (size_t) (element->location.start - erb_content_source);
    size_t end_offset = (size_t) (element->location.end - erb_content_source);

    start = byte_offset_to_position(source, erb_content_offset + start_offset);
    end = byte_offset_to_position(source, erb_content_offset + end_offset);
  }

  hb_array_append(
    entries,
    ast_herb_state_declaration_node_init(
      name_token,
      NULL,
      herb_directive_kind_for_prism_node(NULL),
      start,
      end,
      hb_array_init(0, allocator),
      allocator
...
static hb_array_T* extract_state_entries(
  ...
  for (size_t index = 0; index < hash->elements.size; index++) {
    ...
    if (element->type != PM_ASSOC_NODE || ((pm_assoc_node_t*) element)->key->type != PM_SYMBOL_NODE) {
      append_unusable_state_entry(entries, element, source, erb_content_offset, erb_content_source, allocator);
      continue;
    }
