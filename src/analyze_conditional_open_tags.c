...
static void check_and_report_multiple_tags_in_if(AST_ERB_IF_NODE_T* if_node, hb_arena_T* arena) {
  if (!if_node || !if_node->subsequent) { return; }

  single_open_tag_result_T if_result = get_single_open_tag_from_statements(if_node->statements);

  if (if_result.has_multiple_tags) {
    add_multiple_tags_error_to_erb_node((AST_NODE_T*) if_node, if_result.second_tag, arena);
    return;
  }
  ...
  while (current) {
    ...
    if (branch_result.has_multiple_tags) {
      add_multiple_tags_error_to_erb_node(current, branch_result.second_tag, arena);
      return;
    }
...
static void check_and_report_multiple_tags_in_unless(AST_ERB_UNLESS_NODE_T* unless_node, hb_arena_T* arena) {
  if (!unless_node || !unless_node->else_clause) { return; }

  single_open_tag_result_T unless_result = get_single_open_tag_from_statements(unless_node->statements);

  if (unless_result.has_multiple_tags) {
    add_multiple_tags_error_to_erb_node((AST_NODE_T*) unless_node, unless_result.second_tag, arena);
    return;
  }
  ...
  if (else_result.has_multiple_tags) {
    add_multiple_tags_error_to_erb_node((AST_NODE_T*) unless_node->else_clause, else_result.second_tag, arena);
    return;
  }
}

static void rewrite_conditional_open_tags(hb_array_T* nodes, conditional_open_tags_context_T* context) {
  ...
  for (size_t i = 0; i < hb_array_size(nodes); i++) {
    ...
    if (node->type == AST_ERB_IF_NODE) {
      ...
      if (tag_name) {
        ...
        check_and_report_multiple_tags_in_if(if_node, arena);
      }
      ...
      if (tag_name) {
        ...
        check_and_report_multiple_tags_in_unless(unless_node, arena);
      }
    }
...
static void transform_conditional_open_tags_in_array(hb_array_T* array, conditional_open_tags_context_T* context) {
  ...
  rewrite_conditional_open_tags(array, context);
}
