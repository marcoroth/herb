...
module ERBX
  class Token
    ...
    def inspect
      %(#<ERBX::Token type="#{type}" value="#{value}" range=[#{range.start_position}, #{range.end_position}] start=#{start_location.line}:#{start_location.column} end=#{end_location.line}:#{end_location.column}>)
    end
  end
end
