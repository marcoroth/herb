    ...
    module Slots
      ...
      module RenderBindings
        ...
        def self.collect(state, entries, problems)
          ...
          if name.start_with?(SPLAT)
            problems << ["`state:` spells every binding out as `name: value`, and `#{name}` hides which states it binds.", "Write the states out one by one.", location]

            return
          end

          unless BARE_IDENTIFIER.match?(name)
            problems << ["`#{name}` is not a state name.", "Name each state with a symbol key, like `open: modal_open`.", location]
