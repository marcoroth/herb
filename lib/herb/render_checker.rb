...
module Herb
  class RenderChecker
    ...
    Result = Struct.new(
      :render_calls, :dynamic_calls, :partial_files,
      :unresolved, :unused, :view_root,
      keyword_init: true
    ) do
      def issues?
    ...
    def graph_file!(file_path)
      ...
      reverse_graph = Hash.new { |hash, key| hash[key] = [] }
        ...
        if callers.any?
          ...
          callers.each do |caller_file|
            if caller_file == "__ruby__"
              ...
              puts "   \u251c\u2500\u2500 #{cyan(caller_display)} #{caller_status}"
            end
          end
          ...
          if entry_chains.any?
            entry_chains.each do |chain|
              chain_display = chain.map { |name| cyan(name) }.join(dimmed(" \u2190 "))
              puts "   \u251c\u2500\u2500 #{chain_display}"
            end
          else
    ...
    def collect_render_calls_by_file(files)
      @dynamic_prefixes_from_erb = []
      @layout_refs_from_erb = []
  ...
  class RenderCallVisitor < Visitor
    ...
    def visit_erb_render_node(node)
      ...
      call[:file_path] = node.file&.value if node.file
      call[:inline] = true if node.inline_template
      call[:renderable] = node.renderable&.value if node.renderable
      call[:dynamic] = true if node.dynamic?

      [:body, :plain, :html].each do |field|
        call[field] = true if node.send(field)
      end
