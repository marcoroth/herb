...
module Herb
  module Dev
    ...
    module Protocol
      ...
      #: (file: String, mode: Symbol?, from: String?, to: String?, ?manifest: Hash[String, untyped]?, ?static_markup: String?, ?statics: Hash[String, String]?, ?remap: Hash[String, untyped]?, ?diagnostics: Array[Hash[Symbol, untyped]], ?source: String?) -> Hash[Symbol, untyped]
      def self.schema(file:, mode:, from:, to:, manifest: nil, static_markup: nil, statics: nil, remap: nil, diagnostics: [], source: nil)
        {
          type: "schema",
          file: file,
          mode: mode&.to_s,
      ...
      def self.counterpart?(entry, old, path)
        entry[:node_path] == path && entry[:type] == old[:type] && entry[:name].nil?
      end
