...
import type { LineNumberStyle, MarkerMode, MessageStyle } from "./html-sink.js"
...
export class CLI {
  ...
  private parseRawArguments() {
    ...
    if (["--theme", "--format", "--focus", "--context-lines", "--max-width", "--diagnostics", "--html-markers", "--html-chrome", "--html-messages", "--html-line-numbers", "--html-fragment-separator"].includes(last)) {
      args.pop()
    }
  ...
  private async parseArguments() {
    ...
    for (const flag of ["html-markers", "html-chrome", "html-messages", "html-line-numbers", "html-fragment-separator"] as const) {
    ...
    const htmlLineNumbersValue = values["html-line-numbers"] || "css"

    if (htmlLineNumbersValue !== "css" && htmlLineNumbersValue !== "element") {
      console.error(`Invalid html line numbers mode: ${htmlLineNumbersValue}. Valid modes: css, element.`)
      process.exit(1)
    }

    const htmlLineNumbers = htmlLineNumbersValue as LineNumberStyle
  ...
  private async runDiff(inputs: string[], options: { theme: string, format: string, contextLines: number, showLineNumbers: boolean, wrapLines: boolean, truncateLines: boolean, maxWidth?: number, htmlMarkers: MarkerMode, htmlChrome: string, htmlMessages: MessageStyle, htmlLineNumbers: LineNumberStyle, htmlFragmentSeparator?: string }): Promise<void> {
    const { theme, format, contextLines, showLineNumbers, wrapLines, truncateLines, maxWidth, htmlMarkers, htmlChrome, htmlMessages, htmlLineNumbers, htmlFragmentSeparator } = options
    ...
    if (format === "html" || format === "json") {
      ...
      const sinkOptions = { themeLabel: theme, showLineNumbers, markers: htmlMarkers, diffLayout: "unified" as const, messageStyle: htmlMessages, lineNumberStyle: htmlLineNumbers }
  ...
  async run() {
    const { positionals, diffMode, theme, format, focusLine, contextLines, showLineNumbers, wrapLines, truncateLines, maxWidth, diagnostics, splitDiagnostics, htmlMarkers, htmlChrome, htmlMessages, htmlLineNumbers, htmlFragmentSeparator } =
    ...
    if (diffMode || isDiffSubcommand) {
      ...
      await this.runDiff(inputs, { theme, format, contextLines, showLineNumbers, wrapLines, truncateLines, maxWidth, htmlMarkers, htmlChrome, htmlMessages, htmlLineNumbers, htmlFragmentSeparator })
    ...
    try {
      ...
      if (format === "html") {
        ...
        const sinkOptions = { themeLabel: theme, showLineNumbers, markers: htmlMarkers, messageStyle: htmlMessages, lineNumberStyle: htmlLineNumbers }
