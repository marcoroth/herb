import { STATIC_FLASH_EVENT, STATIC_REMOVED_EVENT } from "../slots/flash"

import { parseMarker, regionCloseMarker, regionOpenMarker } from "@herb-tools/client"
import { reshapeRegion } from "./reshape"
...
function reshapable(region: Region, options: RebuildOptions): boolean {
  if (options.staticsUnsupported) {
    return false
  }

  const moved = region.version !== options.version
...
export function rebuildRegion(runtime: Runtime, options: RebuildOptions): RebuildResult | null {
  ...
  if (reshapable(region, options)) {
    const touched: Node[] = []
    const reshaped = reshapeRegion(runtime.slots, region, {
      version,
      staticMarkup,
      changedStatics: options.changedStatics,
      onMutation: (node) => touched.push(node),
      onRemoval: (rect) => document.dispatchEvent(new CustomEvent(STATIC_REMOVED_EVENT, { detail: { rect } })),
    })

    if (reshaped) {
      for (const node of touched) {
        document.dispatchEvent(new CustomEvent(STATIC_FLASH_EVENT, { detail: { node } }))
      }
      ...
      return { region, restored: 0, dropped: [] }
    }
  }
