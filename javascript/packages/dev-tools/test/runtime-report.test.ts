...
describe("normalizeDiagnostic", () => {
  ...
  test("keeps the origin exactly as the producer wrote it", () => {
    expect(trimOrigin("Herb Parser")).toBe("Herb Parser")
    expect(trimOrigin("herb-parser")).toBe("herb-parser")
    expect(trimOrigin("HERB_PARSER")).toBe("HERB_PARSER")
    expect(trimOrigin("Acme Scanner")).toBe("Acme Scanner")
  })

  test("trims surrounding whitespace and nothing else", () => {
    expect(trimOrigin("Herb Linter ")).toBe("Herb Linter")
    expect(trimOrigin("  Herb Engine Runtime  ")).toBe("Herb Engine Runtime")
    expect(trimOrigin("Herb  Linter")).toBe("Herb  Linter")
  })

  test("falls back to unknown for a missing or empty origin", () => {
    expect(trimOrigin("")).toBe("unknown")
    expect(trimOrigin("   ")).toBe("unknown")
    expect(trimOrigin(undefined)).toBe("unknown")
    expect(trimOrigin(7)).toBe("unknown")
  })

  test("carries the origin into the contract untouched", () => {
    expect(diagnostic({ origin: "Herb Parser" }).origin).toBe("Herb Parser")
    expect(diagnostic({ origin: "Acme Scanner " }).origin).toBe("Acme Scanner")
  })
