...
import { A11yNoTitleAttributeRule } from "../../src/rules/a11y-no-title-attribute.js"
import { createLinterTest } from "../helpers/linter-test-helper.js"

const { expectNoOffenses, expectWarning, assertOffenses } = createLinterTest(A11yNoTitleAttributeRule)

describe("a11y-no-title-attribute", () => {
  ...
  test("fails for button with title attribute", () => {
    expectWarning("The `title` attribute should never be used as it is inaccessible for several groups of users. Use `aria-label` or `aria-describedby` instead. Exceptions are provided for `<iframe>` and `<link>` elements.")
