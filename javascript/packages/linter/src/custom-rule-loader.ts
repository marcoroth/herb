  ...
  async loadRuleFile(filePath: string): Promise<RuleClass[]> {
    try {
      ...
      for (const [exportName, exportValue] of Object.entries(module)) {
        if (exportName !== 'default' && this.isValidRuleClass(exportValue as any)) {
          rules.push(exportValue as RuleClass)
        }
      }

      if (rules.length === 0 && !this.silent) {
        console.warn(`Warning: No valid rule classes found in "${filePath}"`)
      }
