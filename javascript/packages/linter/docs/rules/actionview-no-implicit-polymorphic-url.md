...
Symbols, Strings, plain variables and model reader calls all carry a name the route can be built from. A reader call contributes the name of the reader, so `[:admin, @user.account]` becomes `admin_account_path(@user.account)`. An Array holding anything else is still reported, but without a suggested helper, since the alternative would be guessing at the route name: `[*@parents, @user]` says nothing about where it lands, and `[:admin, User]` routes through `User.model_name.route_key`, which means pluralizing a class name. What the report has to offer there is `polymorphic_path`, which at least says in the template that the route is resolved from the model.

A String is reported even though Rails raises an `ArgumentError` for it and asks for a Symbol, since `["admin", @user]` names the same route the working version does, and `admin_user_path(@user)` is the fix for both problems at once. A method call in the URL argument suggests a helper the same way, built from the name of the reader, so `<%= link_to "Account", @user.account %>` suggests `account_path(@user.account)`. That name is a heuristic in the same way the variable name is. An association named after its role rather than its class, say an `author` association holding a `User`, will suggest `author_path` where the app defines `user_path`, and the route to reach for is the one the app actually defines.
