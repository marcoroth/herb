...
export class FormattingProvider {
  ...
  private async loadConfiguredRewriters() {
    ...
    try {
      const baseDir = this.config.projectPath || this.project.root
      ...
      this.connection.console.log(`[Rewriter] project.root: ${this.project.root}`)
  ...
  private shouldFormatFile(filePath: string): boolean {
    ...
    const relativePath = filePath.replace('file://', '').replace(this.project.root + '/', '')
