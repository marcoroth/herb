...
describe("DefinitionProvider", () => {
  ...
  function createService(...files: string[]) {
    ...
    return new DefinitionProvider(parserService, filePath => existing.has(filePath), () => "")
  }
  ...
  function selection(service: DefinitionProvider, content: string, target: string, uri = VIEW_URI) {
    const document = TextDocument.create(uri, "erb", 1, content)
