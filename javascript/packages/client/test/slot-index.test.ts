...
// What `content_for` produces: part of one rendering, written out somewhere else on the page.
const DISPLACED =
...
describe("HerbRuntime", () => {
  ...
  test("hands back the same runtime rather than a second index", () => {
    const runtime = HerbRuntime.init()

    expect(HerbRuntime.init()).toBe(runtime)
